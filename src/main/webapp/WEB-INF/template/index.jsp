...
<head>
    ...
    <script>
        ...
        //获取广告数据
       /* function advList(){
            $.get("adv/list",function(data){
                showAdvList(data);
            })
        }*/
        //展示广告信息
        /*function showAdvList(data){
            var list = $(".swiper-wrapper");
            list.html("");
            $.each(data,function(index,item){
                var slide = $("#swiper-slide").clone();
                slide.removeAttr("hidden");
                slide.find("a").attr("href",item.linkurl);
                slide.find("img").attr("src","../../upload/"+item.advimg);
                list.append(slide);
            })
        }*/
    </script>
